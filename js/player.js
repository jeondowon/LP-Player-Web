      ...
      onStateChange: (e) => {
        if (e.data === YT.PlayerState.PLAYING) {
          ...
          setTimeout(() => {
            try {
              if (
                S.ytPlayer.getPlayerState() === YT.PlayerState.UNSTARTED &&
                S.isPlaying
              ) {
...
function togglePlay() {
  if (S.currentIndex === -1 && S.playlist.length) {
    playTrack(0);
    return;
  }
...
function nextTrack() {
  if (S.playlist.length) playTrack((S.currentIndex + 1) % S.playlist.length);
}
function prevTrack() {
  if (S.playlist.length)
    playTrack((S.currentIndex - 1 + S.playlist.length) % S.playlist.length);
}
function updPlayBtn() {
  document.getElementById("play-btn").textContent = S.isPlaying ? "⏸" : "▶";
}
function updNpArt() {
...
// ── PLAYLIST ──
function addTrack(t) {
  if (S.playlist.find((x) => x.videoId === t.videoId)) {
...
function renderList() {
  ...
  if (!S.playlist.length) {
    c.innerHTML =
      '<div class="playlist-empty">Search for tracks above<br>and build your playlist.</div>';
    return;
  }
  c.innerHTML = S.playlist
    .map(
      (t, i) => `
  ...
  c.querySelectorAll("[data-r]").forEach((b) =>
    b.addEventListener("click", () => removeTrack(+b.dataset.r)),
  );
  setTimeout(
    () => c.querySelectorAll(".ti-name .marquee-wrap").forEach(applyMarquee),
    50,
  );
}
...
function renderShelf() {
  ...
  c.innerHTML = S.playlist
    ...
    .join("");
  c.querySelectorAll(".shelf-lp").forEach((el) =>
    el.addEventListener("click", () => playTrack(+el.dataset.i)),
  );
  setTimeout(
    () =>
      c.querySelectorAll(".shelf-lp-title .marquee-wrap").forEach(applyMarquee),
    50,
  );
}
function updShelfActive(i) {
  document
    .querySelectorAll(".shelf-lp")
    .forEach((el, j) => el.classList.toggle("active", j === i));
}
...
ST.addEventListener("mousemove", (e) => {
  if (!sd) return;
  e.preventDefault();
  ST.scrollLeft = ssl - (e.pageX - ST.offsetLeft - sdx);
});
...
async function doSearch() {
  ...
  try {
    ...
    SR.querySelectorAll(".si-add").forEach((b) =>
      b.addEventListener("click", (e) => {
        e.stopPropagation();
        addTrack(srData[+b.dataset.i]);
      }),
    );
    SR.querySelectorAll(".search-item").forEach((el, i) =>
      el.addEventListener("click", (e) => {
        if (e.target.classList.contains("si-add")) return;
        addTrack(srData[i]);
      }),
    );
  } catch (err) {
...
document.addEventListener("click", (e) => {
  if (!e.target.closest("#playlist-panel")) SR.style.display = "none";
});
...
function applyMarquee(wrapEl) {
  const text = wrapEl.querySelector(".marquee-text");
  if (!text) return;
...
setTimeout(() => {
  if (!S.ytReady) showToast("Live Server로 실행해 주세요.");
}, 5000);
